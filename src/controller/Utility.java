...
public class Utility {
    ...
    public static void saveToExcel(List<TableSportsman> sportsmen, boolean openFile) {
        ...
        fileLocation = path.substring(0, path.length() - 1) + "temp" + File.separator;
        String excelFileLocation = fileLocation + "sportsman.xlsx";
        ...
        if (openFile) {
            try {
                Desktop.getDesktop().open(new File(excelFileLocation));
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
    ...
    public static void saveToPdf(List<TableSportsman> sportsmen) {
        saveToExcel(sportsmen, false);
        String excelFileLocation = fileLocation + "sportsman.xlsx";
        try {
            com.aspose.cells.Workbook pdfWorkbook = new com.aspose.cells.Workbook(excelFileLocation);
            pdfWorkbook.save(fileLocation + "sportsman.pdf", SaveFormat.PDF);
            Desktop.getDesktop().open(new File(fileLocation + "sportsman.pdf"));
        } catch (Exception e) {
            System.err.println(e.getMessage());
        }
    }
